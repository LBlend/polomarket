...
export const metadata: Metadata = {
  ...
  description:
    "Follow the predictions and donate to vote on the outcome of Mongol Rally 2026.",
};
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="pt-12 pb-10 text-center">
          ...
          <p className="text-neutral-400 max-w-lg mx-auto mb-8">
            Donate on Spleis to cast your vote.
          </p>
          <a
            href={SPLEIS_URL}
            target="_blank"
        ...
        <div className="bg-rally-card border border-rally-border rounded-2xl p-6 mb-10">
          <div className="flex justify-between text-sm mb-3">
            ...
            <span className="text-white font-semibold tabular-nums">
              {raised.toLocaleString("nb-NO", { maximumFractionDigits: 0 })} /{" "}
              {GOAL_KR.toLocaleString("nb-NO")} kr
            </span>
          </div>
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
      {[0, 1, 2].map((i) => (
        <div
          key={i}
          className="bg-rally-card border border-rally-border rounded-2xl p-6 animate-pulse"
        >
          <div className="h-5 bg-neutral-800 rounded w-3/4 mb-5" />
          <div className="space-y-3">
            {[0, 1, 2].map((j) => (
              <div key={j} className="h-8 bg-neutral-800 rounded" />
            ))}
          </div>
        </div>
      ))}
